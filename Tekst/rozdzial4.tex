...
Firma \emph{ARM} powsta³a ok. roku 1990 \cite{Paprocki2009} dziêki
...
których najbardziej popularnymi by³y \emph{ARM7}, \emph{ARM9} i \emph{ARM11}
zastêpowane opecnie przez uk³ady serii \emph{Cortex}.

Przegl±daj±c dokumentacje konkretnych urz±dzeñ cyfrowych wykorzystuj±cych
...
firma \emph{ARM} odnosi siê do nazw architektur (\ref{tab:nazwa-arch-nazwa-rdzenia}).
...
jest uk³ad uznaczony jako \emph{M0}. W za³o¿eniach mia³ byæ jednostk±
32 bitow± stanowi±c± powa¿na konkurencjê dla mikrokomputerów o¶mio
...
(FPGA). Linia \emph{M3} zostanie szczegó³owiej opisana w kolejnym
paragrafie. Najsilniejszym, pod wzglêdem mocy i mo¿liwo¶ci obliczeniowych,
przedstawicielem podrodziny \emph{Cortex-Mx} s± uk³ady sygnowane jako
\emph{M4}. Ich zastosowaniem maj± byæ w za³o¿eniach obszary podobne
do tych, w których u¿ywa siê procesorów DSP, a wiêc cyfrowe przetwarzanie
sygna³ów. W tym celu mikrokontrolery \emph{Cortex-M4} wyposa¿ono w
sprzêtowe bloki (np. koprocesory) pozwalaj±ce realizowaæ obliczenia
charakterystyczne dla procesorów sygna³owych.

Z punktu widzenia programisty najistotniejsz± ró¿nic± miêdzy poszczególnymi
rdzeniami \emph{Cortex-Mx} jest lista rozkazów (rysunek \ref{fig:thumb_2}).
...
do programu zapisanego przy uzyciu listy \emph{Thumb}. Pisz±c program,
...
w pe³ni, lecz tylko w czê¶ci. Teoretycznie architektura harwardzka
pozwala zmniejszyæ wp³yw dostêpu do danych na szybko¶æ wykonywania
siê programu. W praktyce stosunkowo powolne taktowanie pamiêci flash
(do 24 Mhz \cite{Paprocki2009}) mo¿e spowolniæ wykonywanie programu.

Mikrokontrolery z rdzeniem \emph{Cortex-M3} wspieraj± dostêp do danych
z uwzglêdnieniem dwóch sposobów ulo¿enia bajtów, tj. \emph{little
...
endian jest na odwrót. Dziêki temu mo¿na dostosowaæ i zoptymalizowaæ
projektowan± aplikacjê pod kontem w³asnych potrzeb. Przyk³adowo wykorzystanie
w programie komunikacji sieciowej przy pomocy interfejsu Ethernet,
nie bêdzie wymaga³o zmiany kolejno¶ci bajtów na big endian.
...
pe³ny dostêp do wszystkich zasobów rdzenia. W trybie u¿ytkownika,
...
stosu SP (ang. \emph{stack pointer}) i w rzeczywisto¶ci sklada siê
...
domy¶lnie przez przerwania i j±dra systemów operacyjnych pracuj±cych
w trybie uprzywilejowanym,
\item procesowego wska¼nika stosu PSP (ang. \emph{process stack pointer})
- u¿ywanego przez programy u¿ytkownika uruchamiane pod kontrol± systemu
operacyjnego.
\end{itemize}
...
\end{itemize}
Rejestry slu¿± do sterowania rdzeniem oraz do kontroli przebiegu wykonywania
programu. Moga byæ modyfikowane jedynie za pomoc± specjalnych rozkazów
i nie mo¿e to mieæ miejsca w trakcie normalnej pracy mikrokontrolera
\cite{Paprocki2009}.
...
programu, SRAM, zewnêtrzn± pamiêæ RAM. Odpowiedni± mapê pamiêci pokazano
...
dostêpie bitowym (okre¶lanych jako \emph{bit-band}) - jeden w regionie
pamiêci RAM, a drugi w regionie urz±dzeñ peryferyjnych. W pierwszym
przypadku obszar rozpoczyna siê od adresu 0x20000000, za¶ drugi od
0x40000000 {[}¼ród³o{]}. Takie rozwi±zanie pozwala na optymaln± pracê
rdzenia. Normalnie zaby zmieniæ jeden bit, nale¿y odczytaæ warto¶æ
z w³a¶ciwej komórki pamiêci, ustawiæ w niej warto¶æ odpowiedniego
bitu, a nastêpnie tak zmodyfikowan± warto¶æ zapisaæ w pierwotnej komórce.
Dziêki wykorzystaniu dostêpu bitowego, ten sam rezultat mozna uzyskaæ
dziêki dostêpowi do w³a¶ciwej komórki pamiêci w obszarze mapowania
bitów regionu \emph{bit-band}. Jej odpowiedni adres mo¿na wyznaczyæ
dzieki poni¿szym formu³om \cite{bit_band_arm_www}:
...
\end{itemize}
Aby skasowaæ lub ustawiæ okre¶lony bit w regionie bit-band, nale¿y
wpisaæ odpowiednio 0 lub 1 do w³a¶ciwej komórki w obszarze mapowania.
...
\caption{Przerwania ob³ugiwane przez rdzeñ \emph{Cortex-M3} i ich priorytety
...
Pozwala on na obs³ugê piêtnastu przerwañ (wyjatków) systemowych i
240 przerwañ zewnêtrznych. Faktyczna liczba obs³ugiwanych przerwañ
zale¿y od konkretnego mikrokontrolera. Wiêkszo¶æ wyj±tków systemowych
i wszystkie przerwania wewnêtrzne moga mieæ ustalany programowo priorytet
...
widaæ, zdarzeniem o najwy¿szym priorytecie w systemie jest zerowanie
uk³adu.

Mo¿liwo¶ci kontrolera \emph{NVIC} \cite{Paprocki2009}:
\begin{enumerate}
\item Obs³uga przerwañ zagnie¿d¿onych (zwanych te¿ wielopoziomowymi) - je¶li
w trakcie procedury obs³ugi przerwania ISR (ang. \emph{interrupt service
routine}) wyst±pi przerwanie o wy¿szym poziomie, obs³uga bie¿±cego
...
jest pobierany z wektora w pamiêci i nie potrzeby jego programowego
...
priorytet przerwania podczas wykonywania programu. Nie mozna jednak
zmieniæ priorytetu przerwania przed wyj¶ciem z procedury jego obs³ugi,
dziêki czemu unika sie sytuacji wielokrotnego obs³u¿enia tego samego
...
zadania, czyli warto¶ci kluczowych dla niego rejestrów. Inne sposoby
skrócenia opó¼nieñ w obs³udze przerwañ (np. \emph{Tail-Chaining},
przerywanie operacji POP) opisano w literaturze \cite{Paprocki2009}.
\item Maskowanie przerwañ - wszystkie przerwania mog± byæ maskowane na podstawie
...
pamiêci programu przy jednoczesnym wzro¶cie szybko¶ci wykonywania
programu \cite{Paprocki2009}.
...
assembly language}). Pozwala ona w szybszy i czytelniejszy sposób
wykorzystywaæ w programie rozkazy 16- i 32-bitowe.

Zestaw rozkazów \emph{Thumb-2} zawiera operacje, które mog± byæ wykonywane
zarówno jako 16- jak i 32-bitowe. Przyk³adem jest dodawanie liczby
do zawarto¶ci rejestru \cite{Paprocki2009}:

\begin{lstlisting}
ADDS R0, #5 ;Domy¶lnie zostanie u¿yta instrukcja 16-bitowa
ADDS.N R0, #5 ;Jawne u¿ycie rozkazu 16-bitowego (N = Narrow)
ADDS.W R0, #5 ;Jawne u¿ycie rozkazu 32-bitowego Thumb-2 (W = Wide)
\end{lstlisting}


U¿ycie przyrostka \emph{W} lub \emph{N} po kropce za w³a¶ciw± nazw±
rozkazu pozwala wyspecyfikowaæ d³ugo¶æ rozkazu. Mnemoniki pozbawione
sufiksu s± zwykle t³umaczone na instrukcje 16-bitowe.
