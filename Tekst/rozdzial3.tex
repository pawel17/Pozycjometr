...
\chapter{Interfejs SPI}


\section{Wspó³praca mikrokontrolerów z urz±dzeniami zewnêtrznymi.}
...
symetryczne i niesymetryczne. Schemat obu pokazano na rysunku {[}rysunek{]}.
\begin{figure}
\begin{centering}
...
mo¿liwe jest zastosowanie wspólnego przewodu powrotnego {[}¼ród³o{]}.
...
\end{itemize}
Rysunek {[}rysunek{]} pokazuje schemat linii symetrycznej. Sk³ada
siê ona z dwóch przewiodów wiod±cych sygna³y znajduj±ce siê w przeciwfazie
oraz odbiornika z wej¶ciem ró¿nicowym. Dziêki temu, sygna³y o takich
samych przebiegach indukowane w poszczególnych przewodach znosz± siê
po odjêciu na wej¶ciu wzmacniacza. W celu zapewnienia lepszej ochrony
przed zak³óceniami mo¿liwe jest równie¿ skrêcanie przewodów. Zasiêg
linii symetrycznych jest du¿o wiêkszy ni¿ w przypadku linii niesymetrycznych
i osiaga rz±d 1 km {[}¼ród³o{]}.
...
Rysunek {[}rysunek{]} przedstawia uproszczony schemat uk³adu transmisji
szeregowej. Dziêki obecno¶ci dwóch przewodów sygna³owych, mo¿liwa
jest komunikacja w trybie dupleks (ang. \emph{full duplex}), pozwalaj±ca
na jednoczesn± wymianê danych przez komunikuj±ce siê urz±dzenia. Sygna³
nadawany (TxD) wysy³any jest do portu odbieraj±cego dane (RxD). Urz±dzenia
uczestnicz±ce w komunikacji wyposa¿one s± zarówno w nadajnik jak i
odbiornik.
...
(tzw. odcinek bitowy) {[}¼ród³o{]}. S³u¿y do tego sygna³ zegarowy
nadajnika. Odbiornik okre¶la warto¶æ przesy³anych bitów w ¶rodkach
poszczególnych odcinków bitowych. Tak¿e w tym przypadku u¿ywany jest
sygna³ zegarowy (zegar odbiornika). Istniej± dwa rodzaje transmisji
...
Ka¿d± ramkê rozpoczyna nag³ówek (rysunek {[}rysunek{]}), zawieraj±cy
bajty synchronizacji potrzebne do zsynchronizowania zegarów nadajnika
i odbiornika. W celu zapewnienia poprawnego transferu, synchronizacja
...
transmisja tego typu jest realizowana jednokierunkowo {[}¼ród³o{]}.
...
gotowo¶æ do ich odbioru przy pomocy sygna³u gotowo¶ci (ryzunek {[}rysunek{]}).
...
Na rysunku {[}rysunek{]} pokazano przebiegi sygnalów towarzysz±cych
transmisji równoleg³ej. Sygna³ strobu (aktywny poziomem niskim) jest
ustawiany w sytuacji aktywnego sygna³u gotowo¶ci oraz po ustabilizowaniu
siê stanu magistrali danych. Urz±dzenie zewnêtrzne (odbiornik) wykorzystuje
jego zbocze opadaj±ce do zapisu odbieranych danych. W istniej±cych
interfejsach, zarówno szeregowych jak i równoleg³ych wykorzystuje
siê tak¿e inne sygna³y sterujace komunikacj±.
...
przesuwnego (\emph{¬ród³o:} \cite{Krzyzanowski207})}
...
({[}rysunek{]}). Zwykle wysy³any jest najbardziej znacz±cy bit MSB
(ang. \emph{most significant bit}) a odbierany najmniej znacz±cy LSB
(ang. \emph{least significant bit}). Po wype³nieniu rejestrów nastêpuje
obróbka zawartych w nich danych, tj. zapis do pamiêci, przetwarzanie
...
\caption{Przebiegi czasowe odpowiadaj±ce ró¿nym polaryzacjom i fazom sygna³u
taktuj±cego (\emph{¬ród³o:} \cite{spi_pol_ph})}
...
takiego rozwi±zania pokazano na rysunku {[}rysunek{]}. Poniewa¿ wszystkie
wyj¶cia MISO uk³adów \emph{slave} s± po³±czone do wspólnej linii,
powinny byæ one wyjsciami trójstanowymi.
...
urz±dzeniami \emph{slave} (\emph{¬ród³o:} )}
...
\item zegary RTC,
\item karty MMC oraz SD.\end{itemize}
