...
Wspó³praca mikrokomputerów z urz±dzeniami peryferyjnymi odbywa siê
za pomoc± tzw. interfejsów. Przez interfejs rozumie siê urz±dzenie,
które pozwala na komunikacjê i wymianê danych pomiêdzy dwoma innymi
urz±dzeniami. Istniej± ró¿ne kryteria podzia³u interfejsów:
\begin{itemize}
\item ze wzglêdu na istnienie fizycznego po³±cznia miêdzy komunikujacymi
siê urz±dzeniami:
...
gor±cym) oraz masowego, stanowi±cego odniesienie dla sygna³u po obu
stronach toru transmisyjnego. W przypadku wiêkszej liczby sygna³ów
mo¿liwe jest zastosowanie wspólnego przewodu powrotnego \cite{Krzyzanowski207}.
Najwiêksz± wad± linii niesymetrycznej jest jej podatno¶æ na wp³yw
obcych pól elektromagnetycznych. Powoduj± one indukowanie siê zak³óceñ
w przewodzie sygna³owym i powrotnym, które dodaj± siê lub odejmuj±
od w³a¶ciwego sygna³u na wej¶ciu odbiornika. Z tego powodu linie tego
typu stosuje siê do przesy³u informacji na niedu¿e odleg³o¶ci. W celu
zmniejszenia wp³ywu zewnêtrznych pól, stosuje siê nastêpuj±ce zabiegi:
\begin{itemize}
\item skrêcanie przewodów,
\item ekranowanie linii.
\end{itemize}
Linia symetryczna sk³ada siê z dwóch przewiodów wiod±cych sygna³y
znajduj±ce siê w przeciwfazie oraz odbiornika z wej¶ciem ró¿nicowym.
Dziêki temu, sygna³y o takich samych przebiegach indukowane w poszczególnych
przewodach znosz± siê po odjêciu na wej¶ciu wzmacniacza. W celu zapewnienia
lepszej ochrony przed zak³óceniami mo¿liwe jest równie¿ skrêcanie
przewodów. Zasiêg linii symetrycznych jest du¿o wiêkszy ni¿ w przypadku
linii niesymetrycznych i osiaga rz±d 1 km \cite{Krzyzanowski207}.
...
kilku do klikudziesiêciu kilometrów). Tor ¶wiat³owodowy wymaga zastosowania
...
ilo¶ci bitów (ka¿dy swoim torem). Teoretycznie przyspiesza to szybko¶æ
...
budowa (a co zatym idzie wiêkszy koszt) interfejsów równoleg³ych.
Omawiana fakty uzasadniaj± dlaczego wiêkszo¶æ wspó³czesnych urz±dzeñ
cyfrowych wykorzystuje interfejsy i transmisjê szeregow±.
...
multipleksery. W obu przypadkach nale¿y okre¶liæ czas trwania bitu
(tzw. odcinek bitowy) \cite{Krzyzanowski207}. S³u¿y do tego sygna³
zegarowy nadajnika. Odbiornik okre¶la warto¶æ przesy³anych bitów w
¶rodkach poszczególnych odcinków bitowych. Tak¿e w tym przypadku u¿ywany
jest sygna³ zegarowy (zegar odbiornika). Istniej± dwa rodzaje transmisji
szeregowej, ró¿ni±ce siê zale¿no¶ciami sygna³ów zegarowych nadajnika
i odbiornika:
\begin{itemize}
...
W przypadku transmishi asynchronicznej zegary nadajnika i odbiornika
...
nazwê znaku i sk³adaj± siê zwykle od piêciu do o¶miu bitów. Aby odbiornik
móg³ wykryæ pocz±tek oraz koniec nadawania danych, ka¿dy znak poprzedzony
jest tzw. bitem startu oraz zakoñczony tzw. bitami stopu (od jednego
do dwóch). Dodatkowo w celu zapewnienia poprawno¶ci transmisji stosuje
siê równie¿ tzw. bit parzysto¶ci. Wad± transmisji asynchronicznej
jest istnienie czasu martwego, zmniejszaj±cego efektywno¶æ przesy³u
danych. Suma liczby jedynek tworzacych znak wraz z bitem parzysto¶ci
powinna tworzyæ liczbê parzyst±.
...
\end{itemize}
W transmisji synchronicznej zegary nadajnika i odbiornika sa synchronizowane
(st±d nazwa), dziêki czemu nie s± konieczne bity startu i stopu. Dane
przesy³ane grupowane s± w bloki zwane ramkami (ang. \emph{frame}).
Ka¿d± ramkê rozpoczyna nag³ówek (rysunek \ref{fig:Ramka-transmisji-szeregowej-synchr}),
zawieraj±cy bajty synchronizacji potrzebne do zsynchronizowania zegarów
nadajnika i odbiornika. W celu zapewnienia poprawnego transferu, synchronizacja
ta powinna byæ utrzymywana podczas ca³ego procesu komunikacji. Prócz
wspomnianych bajtów, nag³ówek zawiera tak¿e dodatkowe informacje,
np. ilo¶æ przesy³anych danych. Blok danych zawiera w³a¶ciwe dane zgrupowane
jeden za drugim. W celu zapewnienia poprawno¶ci transmisji, s± one
zakoñczone tzw. bajtami kontrolnymi i wewntualnymi bajtami korekcyjnymi
pozwalaj±cymi na naprawê uszkodzonych danych.
...
\subsection{Transmisja równoleg³a}

W transmisji równoleg³ej grupy bitów wysy³ane s± w tym samym czasie
niezale¿nymi liniami danych. Mo¿na powiedzieæ, ¿e w tym przypadku
przesy³ane s± szeregowo grupy bitów (zwykle bajty lub tzw. s³owa).
Dane wysy³ane s± w takt sygna³u zegarowego, zwanego sygna³em strobuj±cym.
Odbiornik wykorzystuje ten sygna³ do zapisu odbieranych danych. Zwykle
transmisja tego typu jest realizowana jednokierunkowo \cite{Krzyzanowski207}.
W przypadku powolnych odbiorników (np. drukarki) stosuje siê dodatkowe
bufory uniezale¿niaj±ce transmisjê od czasu przetwarzania. Aby jednak
dane mog³y byæ przesy³ane, odbiornik musi zasygnalizowaæ nadajnikowi
gotowo¶æ do ich odbioru przy pomocy sygna³u gotowo¶ci (rysunek \ref{fig:Uklad-transmisji-rownoleglej}).

\begin{figure}
\begin{centering}
\includegraphics{rys/interfejsy/transmisja_rownolegla}
\par\end{centering}

\caption{Uk³ad transmisji równoleg³ej (\emph{¬ród³o:} \cite{Krzyzanowski207})
\label{fig:Uklad-transmisji-rownoleglej}}


\end{figure}


Na rysunku \ref{fig:sygnaly-transm-rown} pokazano przebiegi sygnalów
towarzysz±cych transmisji równoleg³ej. Sygna³ strobu (aktywny poziomem
niskim) jest ustawiany w sytuacji aktywnego sygna³u gotowo¶ci oraz
po ustabilizowaniu siê stanu magistrali danych. Urz±dzenie zewnêtrzne
(odbiornik) wykorzystuje jego zbocze opadaj±ce do zapisu odbieranych
danych. W istniej±cych interfejsach, zarówno szeregowych jak i równoleg³ych
wykorzystuje siê tak¿e inne sygna³y sterujace komunikacj±.

\begin{figure}
\begin{centering}
\includegraphics{rys/interfejsy/przebiegi_transmisja_rownolegla}
\par\end{centering}

\caption{Przbiegi sygna³ów w transmisji równoleg³ej (\emph{¬ród³o:} \cite{Krzyzanowski207})
\label{fig:sygnaly-transm-rown}}
...
najczê¶ciej u¿ywanych interfejsów komunikacyjnych miêdzy uk³adami
mikroprocesorowymi a peryferiami takimi jak przetworniki ADC/DAC,
uk³ady RTC, pamiêci EEPROM, pamiêci flash, karty MMC/SD, sterowniki
ekranów dotykowych itp. W ¶wietle omówionych wcze¶niej cech interfejsów,
SPI mo¿e byæ scharaktryzowany jako:
\begin{itemize}
\item szeregowy,
\item synchroniczny,
...
\item przewodowy.
\end{itemize}
Komunikacja z urz±dzeniami wykonywana jest w oparciu o model wymiany
danych master/slave. Do przesy³u danych wykorzystywane s± zwykle 4
przewody, choæ interfejs mo¿e równie¿ funkcjonowaæ w postaci trójprzewodowej.
...
\end{itemize}
Z pojedynczym uk³adem \emph{master} mo¿e wspó³pracowaæ jeden lub wiêcej
uk³adów typu \emph{slave}. W ostatnim przypadku nale¿y zapewniæ odpowiedni±
(równa liczbie urz±dzeñ \emph{slave}) ilo¶æ niezale¿nych sygna³ów
wyboru p³ytki (CS). Poniewa¿ uk³ad transmisji SPI wykorzystuje rejestr
przesuwny, innym rozwi±zaniem mo¿e okazaæ siê tak¿e po³±czenie uk³adów
podrzêdnych, tak aby utworzy³y jeden rejestr przesuwny. Ponadto wspó³praca
z wieloma urz±dzeniami \emph{slave} wymaga, aby w trakcie ich nieaktywno¶ci
ich wyj¶cia MISO wchodzi³y w stan wysokiej impedancji.
...
przesuwnego (\emph{¬ród³o:} \cite{Krzyzanowski207}) \label{fig:spi-rej-przesuwny}}
...
ona wiêksza od ¿adnej z maksymalnych dopuszczalnych czêstotliwo¶æi
...
\item urz±dzenie slave wysy³a klejny bit danych lini± MISO, za¶ master go
odbiera.
\end{itemize}
W transmisji wykorzystywane s± zwykle dwa rejestry przesuwne (po jednym
dla urz±dzenia master i slave) po³±czone ze sob± tworz±c rejestr cykliczny
(rysunek \ref{fig:spi-rej-przesuwny}). Zwykle wysy³any jest najbardziej
znacz±cy bit MSB (ang. \emph{most significant bit}) a odbierany najmniej
znacz±cy LSB (ang. \emph{least significant bit}). Po wype³nieniu rejestrów
nastêpuje obróbka zawartych w nich danych, tj. zapis do pamiêci, przetwarzanie
itp. W celu wymiany wiêkszej ilo¶ci informacji, wspomniany proces
siê powtarza. W trakcie pojedynczego cyklu komunikacji przesy³any
jest zwykle bajt danych (8 bitów), choæ nie jest to regu³±; istniej±
uk³ady w których przesyla siê 12, 16 lub 24 bity. Niektóre urz±dzenia
\emph{slave} nie pozwalaj± na odbieranie danych w ci±gu wiêkszej od
zadanej liczby taktów sygna³u zegarowego; w innych rozwi±zaniach konstrukcyjnych
dane nadmiarowe s± ignorowane.
...
rzeczywistego. Standard SPI nie przewiduje u¿ycia przerwañ, wiêc przeznaczanie
dla nich oddzielnego pinu w urz±dzeniu \emph{slave} nie jest ani zakazane
ani obowi±zkowe.

Ka¿de z urz±dzeñ \emph{slave}, które nie zosta³o aktywowane sygna³em
...
Poza okre¶leniem czêstotliwo¶ci sygna³u taktujacego, urz±dzenie nadrzêdne
...
do wspólnej linii, powinny byæ one wyjsciami trójstanowymi.
...
\begin{itemize}
\item wiêksz± w porównaniu do I2C ilo¶æ pinów obudowy IC,
...
\section{Interfejs UART}

UART (ang. \emph{Universal Asynchronous Receiver and Transmitter})
jest urz±dzeniem umo¿liwiaj±cym asynchroniczne odbieranie i nadawanie
informacji przy pomocy portu szeregowego. Sklada siê z trzech zasadniczych
elementów:
\begin{itemize}
\item konwertera równoleg³o-szeregowego (ang. \emph{parallel to serial}),
pozwalaj±cego na przesy³anie danych z komputera,
\item konwertera szerogowo-równoleg³ego, umo¿liwiaj±cego odbiór danych pochodz±cych
z komputera przez urz±dzenie zewnêtrzne,
\item bufora danych, s³u¿±cego do tymczasowego przechowywania danych w przypadku
szybkiej transmisji.
\end{itemize}
UART u¿ywany jest czêsto w po³aczeniu z takim standardami komunikacyjnymi
...
sygna³ów elektrycznych oraz ich postaæ (np. sygnal ró¿nicowy) s± przez
zewnêtrzny uk³ad sterownika. Omawiane urz±dzenia wyst±puj± jako uk³ady
scalone (b±d¼ ich komponenty) bêd±ce sk³adnikami mikrokontrolerów.
Isniej± konstrukcje ³±cz±ce dwa (DUART) lub osiem uk³adów UART w jednej
obudowie. Istniej± równie¿ chipy pozwalaj±ce na komunikacjê synchroniczn±,
okreslane w skrócie jako USART (ang. \emph{Universal Synchronous and
...
Rysunek \ref{fig:Ramka-danych-uart} przedstawia typow± ramkê u¿ywanê
...
Znak sk³ada siê zazwyczaj z 5 - 8 bitów. W celu zapewnienia kontroli
poprawno¶ci danych, mo¿e byæ zakoñczony tzw. bitem parzysto¶ci. W
celu zasygnalizowania odbiornikowi zakoñczenia transmisji, przesy³any
jest bit stopu, który mo¿e byæ zdublowany.


\subsection{Odbiór danych}

Wszystkie operacje w ukladzie UART sterowane s± sygna³em zegarowym
o czêstotliwo¶æi bêd±cej wielokrotno¶ci± czêsto¶ci u¿ywanej do przesy³ania
...
w chwili zmianu stanu linii danych, o ile zmiana ta mo¿e byæ uznana
...
urz±dzeñ UART.
\begin{itemize}
\item B³±d przepe³nienia (ang. \emph{overrun error}) - wystepuje, gdy odbiorca
danych nie mo¿e przetworzyæ danej przed pojawieniem siê kolejne. Ró¿ne
urz±dzenia dysponuj± ró¿nymi pojemno¶ciami buforów s³u¿±cych do przechowywania
odebranych danych. Jednostka przetwarzaj±ca dane musi to robiæ na
tyle szybko, aby zwolniæ miejsce dla kolejnych znaków - w przeciwnym
razie dochodzi do nadpisania nie przetworzonych jeszcze informacji.
...
jest b³±d ramki.
\item B³±d parzysto¶ci (ang. \emph{parity error}) - wystepuje, gdy liczba
jedynek w przesy³anym znaku wraz z bitem parzysto¶ci jest liczb± parzyst±.
Istnienie omawianego bitu w ramce jest opcjonalne.\end{itemize}
