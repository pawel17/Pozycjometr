...
u8_t L3G4200D_ReadReg(u8_t Reg, u8_t* Data) {
  ...
  static const u8_t scReadBitMask = (1u << 7u);
  u8_t command = scReadBitMask | Reg;

  *Data = SPI_Mems_Read_Reg(command);
...
u8_t L3G4200D_WriteReg(u8_t Reg, u8_t Data) {
  ...
  static const u8_t scWriteBitMask = (1u << 7u);
  u8_t command = (~scWriteBitMask) & Reg;

  SPI_Mems_Write_Reg(command, Data);
